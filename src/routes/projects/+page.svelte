<!-- <script>
  ...
  const brandData = [
    ...
    },
    {
      ...
      subtitle: 'Timothy & Co. — Brand Strategy',
    ...
    },
    {
      ...
      subtitle: 'Predicting Audience vs. Critic Ratings',
      role: 'Data Scientist',
      tags: ['R', 'Logistic Regression', 'EDA'],
      accent: '#F5C518',
      year: '2024',
      logo: null,
      logoSize: '56px',
        ...
        <div class="grid">
          {#each filteredPmUx as project}
            <a href="/projects/{project.slug}" class="card" style="--accent: {project.accent}">
              ...
              {#if project.logo}
                <div class="card__logo">
                  <img src={project.logo} alt="{project.title} logo" style="max-height: {project.logoSize}" />
                </div>
              {/if}
              <h3 class="card__title">{project.title}</h3>
              <p class="card__subtitle">{project.subtitle}</p>
...
<style>
  ...
  /* No fixed height — each logo drives its own size via inline max-height from logoSize data */
  .card__logo {
    margin-bottom: 0.75rem;
    display: flex;
    justify-content: center;
